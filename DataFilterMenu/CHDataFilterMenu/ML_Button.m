...
//    CGRect frame = [self.titleLabel.text boundingRectWithSize:CGSizeMake(MAXFLOAT, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:dictM context:nil];
    ...
    self.titleLabel.frame = CGRectMake(0, ml_btnHeight * 0.55 + ml_labelHeight * 0.45, ml_btnWidth, ml_labelHeight);
...
@implementation Search_Button


- (CGRect)titleRectForContentRect:(CGRect)contentRect{
  
    CGRect rect = CGRectMake(0, 0, contentRect.size.width*0.7, contentRect.size.height);
    return rect;
}
- (CGRect)imageRectForContentRect:(CGRect)contentRect{

    CGRect rect = CGRectMake(contentRect.size.width*0.7, contentRect.size.height/2.5, contentRect.size.height/4, contentRect.size.height/5);
    return rect;
}
